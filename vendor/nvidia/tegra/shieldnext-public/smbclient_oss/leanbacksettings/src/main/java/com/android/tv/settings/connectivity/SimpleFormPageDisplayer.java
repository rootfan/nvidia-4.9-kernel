...
public class SimpleFormPageDisplayer
    ...
    public static final int DISPLAY_TYPE_PREFILLED_TEXT_INPUT = 5;
    ...
    public SimpleFormPageDisplayer(Context context, FragmentManager fragmentManager, int contentViewId) {
        mContext = context;
        ...
        switch (formPageInfo.getDisplayType()) {
            ...
            case DISPLAY_TYPE_TEXT_INPUT:
                return displayTextInput(formPageInfo, titleArgument, descriptionArgument,
                        ...
                        emptyAllowed);
            case DISPLAY_TYPE_PREFILLED_TEXT_INPUT:
                return displayPrefillTextInput(formPageInfo, titleArgument, descriptionArgument,
                        currentFormPage, formPageResultListener, prefill, forward,
                        emptyAllowed);
    ...
    private Fragment displayList(final FormPageInfo formPageInfo, String titleArgument,
        ...
        Fragment fragment = SelectFromListWizardFragment.newInstance(
                ...
                formPageInfo.getChoices(mContext, extraChoices), lastPage == null ? null
                        : (SelectFromListWizardFragment.ListItem) lastPage.getData().getParcelable(RESULT_LIST_ITEM));
        displayFragment(fragment, forward);
        mSelectFromListWizardFragmentListener = new SelectFromListWizardFragment.Listener() {
            @Override
            public void onListSelectionComplete(SelectFromListWizardFragment.ListItem selection) {
    ...
    public void displayFragment(Fragment fragment, boolean forward) {
        ...
        transaction.replace(mContentViewId, fragment).commitAllowingStateLoss();
    }



    private Fragment displayPrefillTextInput(FormPageInfo formPageInfo, String titleArgument,
                                      String descriptionArgument, final FormPage formPage,
                                      final FormPageResultListener listener, String prefill, boolean forward,
                                      final boolean emptyAllowed) {
        Fragment fragment = TextInputWizardFragment.newInstance(
                getTitle(formPageInfo, titleArgument),
                getDescription(formPageInfo, descriptionArgument),
                formPageInfo.getInputType(), prefill);
        displayFragment(fragment, forward);
    ...
    private Fragment displayLoading(FormPageInfo formPageInfo, String argument,
        ...
        Fragment fragment = MessageWizardFragment.newInstance(
                getTitle(formPageInfo, argument), null, showProgress);
        displayFragment(fragment, forward);
        return fragment;
    }
