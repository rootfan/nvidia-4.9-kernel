...
class DialogActionAdapter extends RecyclerView.Adapter {
    ...
    public DialogActionAdapter(DialogFragment.Action.Listener listener, DialogFragment.Action.OnFocusListener onFocusListener,
                               List<DialogFragment.Action> actions) {
        super();
        mListener = listener;
        mActions = new ArrayList<DialogFragment.Action>(actions);
    ...
    public DialogFragment.Action getItem(int position) {
        return mActions.get(position);
    }

    public void setListener(DialogFragment.Action.Listener listener) {
        mListener = listener;
        mActionOnKeyPressAnimator.setListener(listener);
    }

    public void setOnFocusListener(DialogFragment.Action.OnFocusListener onFocusListener) {
     ...
     */
    public ArrayList<DialogFragment.Action> getActions() {
        return new ArrayList<DialogFragment.Action>(mActions);
    }
    ...
    private static class ActionViewHolder extends ViewHolder {
        ...
        private boolean setIndicator(final ImageView indicatorView, DialogFragment.Action action) {
    ...
    private static class ActionOnKeyPressAnimator implements View.OnKeyListener {
        ...
        public ActionOnKeyPressAnimator(DialogFragment.Action.Listener listener,
                                        List<DialogFragment.Action> actions) {
            mListener = listener;
            mActions = actions;
        }
        ...
        private void fadeCheckmarks(final View v, final DialogFragment.Action action, int duration, int delay,
                                    Interpolator interpolator) {
            int actionCheckSetId = action.getCheckSetId();
            if (actionCheckSetId != DialogFragment.Action.NO_CHECK_SET) {
                ...
                for (int i = 0, size = mActions.size(); i < size; i++) {
                    DialogFragment.Action a = mActions.get(i);
