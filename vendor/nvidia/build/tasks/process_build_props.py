...
def parseCmdline(logger):
    ...
    parser.add_option("--vendor", action="store_true", help="Pass this for vendor image")
...
def getBuildFingerprint(logger, sObj, pObj, vendor=False):
    """
    Generate build fingerprint based on data in sku manifest
    """
    buildFingerprint = {}

    propName = ['ro.build.fingerprint', 'ro.bootimage.build.fingerprint']
    if vendor:
        propName.append('ro.vendor.build.fingerprint')

    for prop in propName:
        oldVal = pObj.get(prop)
        if (oldVal != None and isinstance(oldVal, basestring)):
            break;

    regExp = r'(?P<brand>.+?)/(?P<product>.+?)/(?P<device>.+?)(:.+)'
    regObj = processRegExp(regExp, oldVal)
    if regObj:
        ...
        oldVal = trio+regObj.group(4)

    for prop in propName:
         buildFingerprint[prop] = oldVal
...
def processRegExp(regExp, val):
    """
    Process a regex value and return the object or none if invalid
    """
    ret = None
    if (val != None and isinstance(val, basestring)):
         ret = re.compile(regExp).match(val)
...
def main():
    ...
    properties.update(getBuildFingerprint(logger, skuObj, propObj, vendor=options.vendor))
    if not options.vendor:
