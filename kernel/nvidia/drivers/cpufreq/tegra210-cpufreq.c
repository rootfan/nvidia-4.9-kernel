...
static int update_cpu_freq(u32 rate, u32 cpu)
{
	int ret = 0;
        ...
        previousClock = get_cpu_freq(0);         
	ret = clk_set_rate(tfreq_priv->cpu_clk, IN_HZ(rate));
        if(ret < 0)
        printk("Failed to update frequency: Previous freq: %u requested: %u result %i\n",previousClock,rate,ret);
