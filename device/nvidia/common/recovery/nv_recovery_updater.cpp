...
Value* NvOtaFileCheck(const char* name, State* state,
  ...
  if (ReadArgs(state, argv, 2, &zip_path, &mount_path) < 0) {
    return ErrorAbort(state, "%s: could not successfully parse the given arguments.", name);
  }
  ...
  entry = mzFindZipEntry(za, zip_path);
  if (!entry) {
    return ErrorAbort(state, "%s: no %s in the package.", name, zip_path);
  }

  // Check the file ota flow needs
  for (i = 0; i < sizeof(nv_otafile_list)/sizeof(*nv_otafile_list); i++) {
    if (!FileExists(nv_otafile_list[i])) {
       return ErrorAbort(state, "%s: no %s in the device.", name, nv_otafile_list[i]);
    }
  }
  ...
  if (GetDevicePath_UnifiedFstab(name, mount_path, block_device_path)) {
    if (GetDevicePath_NonUnifiedFstab(name, mount_path, block_device_path)) {
      return ErrorAbort(state, "%s: couldn't get the block device path for: %s.", \
              name, mount_path);
    }
  }
