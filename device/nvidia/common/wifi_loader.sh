...
if ! /vendor/bin/cmp -s $datafile $etcfile ; then
	/vendor/bin/log -t "wifiloader" -p i  "Linking Scan config file"
	/vendor/bin/rm $datafile
	/vendor/bin/ln -s $etcfile $datafile
fi

if [ "`cat /proc/device-tree/brcmfmac_pcie_wlan/status`" = "okay" ]; then
       ln -sf /sys/module/brcmfmac/parameters/alternative_fw_path /data/misc/wifi/fw_path
       ln -sf /sys/module/brcmfmac/parameters/alternative_fw_path /data/misc/wifi/op_mode
else
       ln -sf /sys/module/bcmdhd/parameters/firmware_path /data/misc/wifi/fw_path
       ln -sf /sys/module/bcmdhd/parameters/op_mode /data/misc/wifi/op_mode
fi
       chmod  0660 /data/misc/wifi/fw_path
       chmod  0660 /data/misc/wifi/op_mode
       chown  wifi:wifi /data/misc/wifi/fw_path
       chown  wifi:wifi /data/misc/wifi/op_mode
